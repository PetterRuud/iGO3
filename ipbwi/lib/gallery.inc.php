<?php
	...
	class ipbwi_gallery extends ipbwi {
		...
		public function __construct($ipbwi){
			...
			$query = self::$ips->DB->query('SELECT conf_value,conf_default FROM ibf_conf_settings WHERE conf_key="gallery_images_url"');
			if(self::$ips->DB->get_num_rows($query) != 0){
				$data = self::$ips->DB->fetch_row($query);
		 ...
		 * @sample
		 * <code>
		 * $ipbwi->gallery->getReadable();
		 * </code>
		 * @since			2.0
		 */
		public function getViewable(){
			if($cache = $this->ipbwi->cache->get('galleryGetViewable', self::$ips->member['id'])){
				return $cache;
			}else{
				self::$ips->DB->query('SELECT * FROM ibf_gallery_categories');
				$cats = array();
				...
				$this->ipbwi->cache->save('galleryGetViewable', self::$ips->member['id'], $cats);
				return $cats;
			}
		}
		/**
		...
		public function getLatestList($catIDs=false,$settings=array()){
			if($this->installed === true){
				...
				// get latest images
				$query = self::$ips->DB->query('SELECT * FROM ibf_gallery_images WHERE approved="1"'.$catquery.$fromMember.' ORDER BY id DESC LIMIT '.intval($settings['start']).','.intval($settings['limit']));
				if(self::$ips->DB->get_num_rows($query) == 0){
					return false;
				}
				$data = array();
			...
			if($this->installed === true){
				...
				if(is_array($cats) && count($cats) > 0){
					foreach($cats as $i){
						...
						if($subqery = self::$ips->DB->query('SELECT '.$select.' FROM ibf_gallery_categories WHERE parent = '.$i['id'].' ORDER BY c_order ASC')){
		...
		public function catList(){
			if($this->installed === true){
				if($cache = $this->ipbwi->cache->get('listGalleryCategories', '1')){
					...
					self::$ips->DB->query('SELECT * FROM ibf_gallery_categories WHERE parent = "0"');
					$cat = array();
					while($row = self::$ips->DB->fetch_row()){
						$row['last_pic_name'] = $this->ipbwi->properXHTML($row['last_pic_name']);
						$row['name'] = $this->ipbwi->properXHTML($row['name']);
		...
		public function info($imgID){
			if($this->installed === true){
				// get image info
				$query = self::$ips->DB->query('SELECT * FROM ibf_gallery_images WHERE id="'.intval($imgID).'"');
				if(self::$ips->DB->get_num_rows($query) == 0){
					return false;
				}
				$data = array();
