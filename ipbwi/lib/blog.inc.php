<?php
	...
	class ipbwi_blog extends ipbwi {
		...
		public function __construct($ipbwi){
			...
			$query = self::$ips->DB->query('SELECT conf_value,conf_default FROM ibf_conf_settings WHERE conf_key="blog_online"');
		...
		public function getLatestList($blogIDs=false,$settings=array()){
			if($this->installed === true){
				...
				$query = self::$ips->DB->query('SELECT e.*,b.* FROM ibf_blog_entries e LEFT JOIN ibf_blog_blogs b ON (b.blog_id=e.blog_id) WHERE e.entry_status="published"'.$blogQuery.' ORDER BY e.entry_id DESC LIMIT '.intval($settings['start']).','.intval($settings['limit']));
				if(self::$ips->DB->get_num_rows($query) == 0){
					return false;
				}
				$data = array();
