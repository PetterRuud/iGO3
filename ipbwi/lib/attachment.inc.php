<?php
	...
	class ipbwi_attachment extends ipbwi {
		...
		public function __construct($ipbwi){
			...
			$sql_select = self::$ips->DB->query($sql_select);
			$oldHash = self::$ips->DB->fetch_row($sql_select);
		...
		public function info($IDs,$settings=array(),$bypassPerms=false){
			...
			if($module === false){
				...
				$query = self::$ips->DB->query('SELECT * FROM ibf_attachments WHERE ('.$dbID.')'.$approved.' ORDER BY attach_date DESC');
			}else{
				// more than one IDs?
				if(is_array($IDs)){
					$dbID = '';
				...
				$query = self::$ips->DB->query('SELECT * FROM ibf_attachments WHERE attach_rel_module = "'.$module.'"'.$approved.' AND ('.$dbID.') ORDER BY attach_date DESC');
			}
		...
		private function copyFile($fileName,$postID){
			if($this->ipbwi->member->isLoggedIn()){
				...
				if(in_array($_POST['file_select'],$files)){
					...
					self::$ips->DB->query('SELECT atype_post FROM ibf_attachments_type WHERE atype_extension = '.$fileExt);
