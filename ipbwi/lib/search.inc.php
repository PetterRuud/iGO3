<?php
	...
	class ipbwi_search extends ipbwi {
		...
		public function results($searchID){
			...
			self::$ips->DB->query('SELECT * FROM ibf_search_results WHERE id="'.$searchID.'"');
			if($row = self::$ips->DB->fetch_row()){
				$searchinfo = $row;
				$tomato = stripslashes($row['query_cache']);
				self::$ips->DB->query($tomato);
				$results = array();
		...
		public function simple($string, $forums = '*', $dateorder = false, $recursive = false){
			...
			self::$ips->DB->query('SELECT VERSION() AS version');
			if(!$row = self::$ips->DB->fetch_row()){
				self::$ips->DB->query('SHOW VARIABLES LIKE "version"');
			...
			self::$ips->DB->query('SELECT COUNT(*) as count FROM ibf_posts p WHERE p.topic_id IN ('.$topics.') AND MATCH(post) AGAINST ("'.$string.'" '.(($version >= '40010') ? 'IN BOOLEAN MODE' : '').')');
			$row = self::$ips->DB->fetch_row();
			...
			self::$ips->DB->query('INSERT INTO ibf_search_results (id, search_date, topic_id, topic_max, member_id, ip_address, post_id, query_cache) VALUES("'.$searchid.'", "'.time().'", "'.$topics.'", "'.$row['count'].'", "'.self::$ips->member['id'].'", "'.self::$ips->input['IP_ADDRESS'].'", NULL, "'.addslashes($store).'")');
