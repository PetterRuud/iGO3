<?php
	...
	class ipbwi_tagCloud extends ipbwi {
		...
		public function view($category=false,$link='?tag=%key%'){
			...
			$query = self::$ips->DB->query('SELECT * from '.ipbwi_DB_prefix.'tagcloud'.$category);
		...
		public function getTagData($tag){
			$query = self::$ips->DB->query('SELECT * from '.ipbwi_DB_prefix.'tagcloud WHERE tag="'.$tag.'"');
			if(self::$ips->DB->get_num_rows($query) == 0){
				return false;
			}
			$data = '';
		...
		public function getTagList($topicID){
			$query = self::$ips->DB->query('SELECT * FROM '.ipbwi_DB_prefix.'tagcloud WHERE tid="'.intval($topicID).'"');
			if(self::$ips->DB->get_num_rows($query) == 0){
				return false;
			}
			$data = array();
		...
		public function getCategoryList(){
			$query = self::$ips->DB->query('SELECT DISTINCT category FROM '.ipbwi_DB_prefix.'tagcloud');
			if(self::$ips->DB->get_num_rows($query) == 0){
				return false;
			}
			$data = array();
		...
		public function addTag($tag,$destination,$topicID=false,$title=false,$category='default'){
			...
			self::$ips->DB->query('INSERT INTO '.ipbwi_DB_prefix.'tagcloud (tag,destination,tid,title,category) VALUES("'.$tag.'","'.$destination.'",'.$topicID.','.$title.',"'.$category.'")');
			return true;
		}
		/**
		...
		public function deleteTag($tagID){
			...
			self::$ips->DB->query('DELETE FROM '.ipbwi_DB_prefix.'tagcloud WHERE id="'.$tagID.'"');
			return true;
		}
	}
?>